...
interface OverlayDataProps {
  data: TraiteData;
  /** Décalage de calibrage en px (Mode 2 uniquement, sinon 0). */
  dx: number;
  dy: number;
}
...
function Field({ pos, children, wrap = false }: { pos: FieldPos; children: ReactNode; wrap?: boolean }) {
  return (
    <div
...
function RibCells({ pos, value, count, size = 3.2 }: { pos: FieldPos; value: string; count: number; size?: number }) {
 ...
 */
export function OverlayData({ data, dx, dy }: OverlayDataProps) {
  const rib = splitRib(data.rib);
  const montantStr = data.montant != null ? montantBorne(data.montant, data.monnaie || 'DT') : '';
  const lettres =
    data.montant != null ? montantEnLettres(data.montant, data.monnaie || 'DT') : '';
  const lettresSize = lettres.length > 95 ? 2.1 : lettres.length > 70 ? 2.5 : 2.9;
  ...
  return (
    ...
    >
      ...
      <Field pos={FIELDS.echeanceBottom}>{formatDateFr(data.dateEcheance)}</Field>
